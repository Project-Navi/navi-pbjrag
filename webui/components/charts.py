...
def phase_bar_chart(chunks: List[Dict[str, Any]]) -> go.Figure:
    ...
    # Create bar chart
    df = pd.DataFrame({"Phase": list(phase_counts.keys()), "Count": list(phase_counts.values())})
...
def epc_timeline_chart(chunks: List[Dict[str, Any]]) -> go.Figure:
    ...
    fig = px.line(df, x="Chunk", y="EPC", title="Evolutionary Path Clarity Timeline", markers=True)
